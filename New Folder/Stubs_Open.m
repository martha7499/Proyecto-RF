classdef Stubs_Open
   ...
   % de los datos de stubs open                                         %
   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    properties
        ang_LE %ángulo electrico 
        frequencie_work %frecuencía de trabajo
        g %matriz de valores para el stubs en el analisi de frecuencía
    end
    methods
        function obj=Stubs_Open(Arg5,k1,Arg4,Arg3,range_frequenci)
             if contains(Arg5{k1},'G')%conocer en que frecuencía trabaja
                ...
                %obj.g{1,n}=vpa(simplify(1/(2*pi*((Arg3{k1})*(1/tan(obj.ang_LE*(range_frequenci(1,n)/obj.frequencie_work)))))),4);%realizar la ecuación
                obj.g=vpa(simplify((2*pi*((Arg3{k1})*(1/tan(obj.ang_LE*(obj.frequencie_work/obj.frequencie_work)))))),4);%realizar la ecuación
                %end
            elseif contains(Arg5{k1},'M')
                ...
                obj.g=cell(1,length(range_frequenci));%generar la matriz para guardar los datos
                %for n=1:length(range_frequenci)%recorrer todas el analisis de frecuencias
                %obj.g{1,n}=vpa(simplify(1/(2*pi*((1/Arg3{k1})*(1/tan(obj.ang_LE*(range_frequenci(n)/obj.frequencie_work)))))),4);%realizar la ecuación  
                obj.g=vpa(simplify((2*pi*((Arg3{k1})*(1/tan(obj.ang_LE*(obj.frequencie_work/obj.frequencie_work)))))),4);%realizar la ecuación
                %end
