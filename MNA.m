...
fprintf('Netlist:');
%type(fname)
fname = "CircuitsExamples\example3.cir"
fid = fopen(fname)
    ...
    switch Name{k1}(1)
        ...
        case 'I' % IXXXXXXX N1 N2 VALUE  (Current N1 to N2)
            ...
            if n2~=0
                ...
                i{n2} = [i{n2} ' + ' Name{k1}]; % add current to n2 %Modificacion corriente
            end
...
% C1 = 1e-9;  % Capacitancia (Faradios)
% L1 = 1e-6;  % Inductancia (Henrios)
% 
% % Definir rango de frecuencias
% f_min = 1e3; % Frecuencia mínima (Hz)
% f_max = 1e6; % Frecuencia máxima (Hz)
% f_step = 1e3; % Paso de frecuencia (Hz)
